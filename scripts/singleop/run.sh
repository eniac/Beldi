...
ENDPOINT="$bp" ./tools/wrk -t1 -c1 -d600s -R1 -s ./benchmark/singleop/workload.lua --timeout 10s "$bp" >/dev/null
echo "Running beldi"
ENDPOINT="$p" ./tools/wrk -t1 -c1 -d600s -R1 -s ./benchmark/singleop/workload.lua --timeout 10s "$p" >/dev/null
echo "Running beldi-txn"
ENDPOINT="$tp" ./tools/wrk -t1 -c1 -d600s -R1 -s ./benchmark/singleop/workload.lua --timeout 10s "$tp" >/dev/null
